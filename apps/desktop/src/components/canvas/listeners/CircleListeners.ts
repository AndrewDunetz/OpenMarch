...
/**
 * CircleListeners is an extension of DefaultListeners that handles the creation of circles on the canvas
 */
export default class CircleListeners
    ...
    implements CanvasListeners
{
    ...
    drawNewMarcherPaths() {
        ...
        if (this.canvas.eventMarchers.length < 6) {
            console.error(
                `Cannot create a marcherLine of < 6 marchers. Only found ${this.canvas.eventMarchers.length}`,
        ...
        const newDots = this._activeLine.distributeMarchersCircle(oldDots);
